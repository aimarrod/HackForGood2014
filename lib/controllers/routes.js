...
exports.addSubscriptor=function(req, res, next){
  ...
  Route.findById(routeId, function (err, route) {
    ...
    route.save(function(err) {
        ...
        var newState = new State({routeName: route.routeName,pointNumber: 1, user:req.user, username:req.user.name});
...
exports.getNearby = function (req, res, next) {
  ...
  var route;
  var nearQuestions=[];
  Route.find({assigned_routes: req.user.name},function (err, routes) {
    for(var i=0; i<routes.length;i++){
      routeFeatures=routes[i].features;
      State.find({$and: [{routeName: routeName, username: user}]}, function(err, states){
        for(var j=0; j<states.length;j++){
            nearQuestions.push(routeFeatures[j]);
        }
      });
    }
  });
